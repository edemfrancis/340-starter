...
async function sendMessage(req, res) {
	...
	// regResult is the result object returned by pg. Check rowCount to confirm INSERT succeeded.
	if (regResult && regResult.rowCount && regResult.rowCount > 0) {
		...
		res.status(201).render("inbox/sendMessageAgain", {
			title: "Send Messages ",
			nav,
			archivedMessageCount,
			inboxMessages,
		...
		console.error("sendNewMessage did not insert any rows:", regResult);
...
async function replyMessage(req, res) {
	...
	if (regResult && regResult.rowCount && regResult.rowCount > 0) {
		...
		console.error("reply sendNewMessage did not insert any rows:", regResult);
